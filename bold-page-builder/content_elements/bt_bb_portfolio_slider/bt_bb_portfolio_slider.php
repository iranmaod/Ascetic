...
class bt_bb_portfolio_slider extends BT_BB_Element {
	...
	function handle_shortcode( $atts, $content ) {
		...
		$show = array( 'title' => false,'category' => false, 'excerpt' => false,  'read_more' => false,'author' => false, 'date' => false,
	...
	static function slider_content($posts, $show){
		...
		foreach( $posts as $item ) {
				...
				if ( $img_src ) {
					...
					$pf_slider_image_default_id = attachment_url_to_postid( boldthemes_get_option( 'pf_image_default' ) );
				...
				$output .= '<div class="bt_bb_grid_item" data-hw="' . esc_attr( $hw ) . '"' . $style_attr . '>'
						. '<div class="bt_bb_grid_item_inner' . esc_attr( implode( ' ', $class ) ) . '"' . $background_color_style . '>';
						if ( $show['image'] ) {
							if ( $show['image_position'] == 'image_above_content' ) {
								...
								$output .= '<div class="bt_bb_grid_item_post_thumbnail">'
									. '<a href="' . esc_url_raw( $item['permalink'] ) . '" target="' . esc_attr( $show['target'] ) . '" title="' . esc_attr( $item['title'] ) . '" style="background-image: url(' . esc_url_raw( $img_src ) . ');">'
									. '</a>'
								. '</div>';
							}
						}
					$output .= '<div class="bt_bb_grid_item_inner_content' . esc_attr( implode( ' ', $class ) ) . '"><div class="arrow"></div>';								
									...
									$meta_output = '<div class="bt_bb_grid_item_meta">';
										...
										if ( $show['date'] ) {
											$meta_output .= '<span class="bt_bb_grid_item_date">';
												$meta_output .= get_the_date( '', $item['ID'] );
											$meta_output .= '</span>';
										}
								...
								if ( ($show['read_more'] && $show['read_more_text'] != '') || ( $show['read_more_icon'] != '' && $show['read_more_icon'] != 'no_icon'  ) ) {
									...
									if ( $read_more_icon_color_scheme_colors ) $read_more_icon_style .= ' --portfolio-slider-read-more-icon-primary-color:' . $read_more_icon_color_scheme_colors[0] . '; --portfolio-slider-read-more-icon-secondary-color:' . $read_more_icon_color_scheme_colors[1] . ';';
									...
									$output .= '<div class="bt_bb_grid_item_read_more' . $read_more_icon_class . '"' . $read_more_icon_style_attr . '>';
											$output .= '<a href="' . esc_url_raw( $item['permalink'] ) . '">' . $read_more_text_html . $icon_html . '</a>';
									$output .= '</div>';
								}
	...
	function map_shortcode() {
		...
		bt_bb_map( $this->shortcode, array( 'name' => esc_html__( 'Portfolio Slider', 'ajani' ), 'description' => esc_html__( 'Slider with posts', 'ajani' ), 'container' => 'vertical',  'toggle' => true, 'icon' => $this->prefix_backend . 'icon' . '_' . $this->shortcode,
			'params' => array(
